...
class BertPooler(nn.Module):
    ...
    def forward(self, hidden_states):
        # We "pool" the embedding_model by simply taking the hidden state corresponding
        # to the first token.
...
class BertPreTrainedModel(nn.Module):
    ...
    def __init__(self, config, *inputs, **kwargs):
        ...
        #         "To create a embedding_model from a Google pretrained embedding_model use "
        #         "`embedding_model = {}.from_pretrained(PRETRAINED_MODEL_NAME)`".format(
...
class BertModel(BertPreTrainedModel):
    """
    The embedding_model can behave as an encoder (with only self-attention) as well
    as a decoder, in which case a layer of cross-attention is added between
    the self-attention layers, following the architecture described in `Attention is all you need`_ by Ashish Vaswani,
    Noam Shazeer, Niki Parmar, Jakob Uszkoreit, Llion Jones, Aidan N. Gomez, Lukasz Kaiser and Illia Polosukhin.
    To behave as an decoder the embedding_model needs to be initialized with the
